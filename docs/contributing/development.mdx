...
<Steps>
  ...
  <Step title="Run Database Migrations If Needed">
    ...
    ```bash
    pnpm exec dotenvx run -f .env.local -- pnpm run db:migrate
    ```
  </Step>
  
  <Step title="Start the Development Server">
    ```bash
    pnpm exec dotenvx run -f .env.local -- pnpm run dev
    ```
...
| Command                        | Description                                                 |
| ------------------------------ | ----------------------------------------------------------- |
| `pnpm exec dotenvx run -f .env.local -- pnpm dev` | Start the web and server development processes   |
...
| Command                | Description                                  |
| ---------------------- | -------------------------------------------- |
| `pnpm exec dotenvx run -f .env.local -- pnpm run db:generate` | Generate migration files from schema changes |
| `pnpm exec dotenvx run -f .env.local -- pnpm run db:migrate`  | Apply pending migrations                     |
| `pnpm exec dotenvx run -f .env.local -- pnpm run db:studio`   | Open Drizzle Studio (database GUI)           |
...
```bash
pnpm exec dotenvx run -f .env.local -- pnpm run db:studio
```
...
2. Generate a migration:
   ```bash
   pnpm exec dotenvx run -f .env.local -- pnpm run db:generate
   ```
...
<AccordionGroup>
	<Accordion title="Port 3000 or 3001 is already in use">
		...
		```bash
		PORT=3002 SERVER_PORT=3003 pnpm exec dotenvx run -f .env.local -- pnpm dev
		```
	</Accordion>
