...
function getModel(input: GetModelInput) {
  ...
  return match(provider)
    ...
    .with("openai-compatible", () => createOpenAI({ apiKey, baseURL }).chat(model))
...
async function testConnection(input: TestConnectionInput): Promise<boolean> {
  ...
  if (input.provider === "openai-compatible") {
    // Avoid structured output features for maximum compatibility across OpenAI-compatible providers.
